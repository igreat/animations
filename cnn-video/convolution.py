...
class Convolution(ThreeDScene):
    def construct(self):
        ...
        self.set_camera_orientation(phi=1 / 9 * TAU, theta=-1 / 12 * TAU)
        ...
        input_channel = get_pixels_grid(orange_array).set_stroke(width=4).shift(IN * 3)
        ...
        kernel = (
            ...
            .set_stroke(width=4)
            .move_to(input_channel.get_corner(DL))
            .shift(UP * kernel_size[1] / 2, RIGHT * kernel_size[0] / 2)
            .set_opacity(0.5)
        )
        kernel.z_index = 1

        input_highlight = Square(1, color=colors.BLACK, fill_opacity=0.5).move_to(
            output_channel[-output_size[0]].get_center()
        )
        ...
        for i in range(output_size[0]):
            ...
            for _ in range(output_size[1] - 1):
                ...
                self.play(
                    kernel_and_highlight.animate.shift(UP * scale_factor),
                )
            # shifting vertically
            if i != output_size[0] - 1:
                self.play(
                    kernel_and_highlight.animate.shift(
                        DOWN * (output_size[1] - 1) * scale_factor, RIGHT * scale_factor
                    ),
                )
