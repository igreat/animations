...
def get_pixels_grid_with_nums(
    ...
    average_pixels = image_array.mean(axis=2).astype(np.uint8)

    # think about using .scale instead of font_size
    labels = [
        Text(
        ...
        for square, x in zip(pixel_grid, average_pixels.flatten())
    ]
